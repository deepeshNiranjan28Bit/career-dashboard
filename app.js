...
function renderTestFiles() {
  const settings = Store.settings();
  const bundled = bundledFiles('TEST_FILES');
  const files = settings.testFiles || [];
  let html = '<div class="card"><h2>Test files</h2>' +
    '<div class="grid cols-2">';
  bundled.forEach(function (f) { html += linkCard(f.name, f.desc || 'Mock test.', 'tests/' + f.file); });
  // Extra files registered on this device only (not part of the deployment).
  const bundledNames = bundled.map(function (f) { return f.file; });
  files.filter(function (f) { return bundledNames.indexOf(f.file) < 0; })
    .forEach(function (f) { html += linkCard(f.name, 'Registered on this device.', 'tests/' + f.file); });
  html += '</div>';
  if (!bundled.length && !files.length) html += '<div class="empty">No test files yet.</div>';
  html += '<div class="faint" style="margin:12px 0 6px">Drop new test HTML files into the <span class="mono">tests/</span> folder, run <span class="mono">node sync-content.js</span>, then commit &amp; push — they appear on every device. The box below only registers a file on <i>this</i> device.</div>';
...
// Files bundled with the site (from content-config.js, generated by sync-content.js).
// These show on EVERY device without manual registration.
function bundledFiles(varName) {
  const arr = (typeof window !== 'undefined' && window[varName]) ? window[varName] : [];
...
function viewLibrary() {
  ...
  const bundled = bundledFiles('LIBRARY_FILES');
  html += '<div class="card"><div class="grid cols-2">';
  bundled.forEach(function (f) { html += linkCard(f.name, f.desc || 'Study guide.', 'library/' + f.file); });
  const bundledNames = bundled.map(function (f) { return f.file; });
  extra.filter(function (f) { return bundledNames.indexOf(f.file) < 0; })
    .forEach(function (f) { html += linkCard(f.name, 'Registered on this device.', 'library/' + f.file); });
  html += '</div>';
  if (!bundled.length && !extra.length) html += '<div class="empty">No study guides yet.</div>';
  html += '<div class="faint" style="margin:12px 0 6px">Add new study files into the <span class="mono">library/</span> folder, run <span class="mono">node sync-content.js</span>, then commit &amp; push — they appear on every device. The box below only registers a file on <i>this</i> device.</div>';
